...
void access_l2(const uint32_t address, uint8_t* data, access_mode mode) {
    uint32_t tag, line_index, mem_address;
    ...
    mem_address =
        tag << (L2_TAG_OFFSET) | (line_index << (BYTE_OFFSET + BLOCK_OFFSET));

    cache_line_t* line = &(l2_simple_cache.lines[line_index]);
    ...
    if (mode == MODE_READ) {
        ...
        memcpy(data,
               &(l2_cache[l2_index_plus_word_to_addr(
                   line_index, (address >> BYTE_OFFSET) % WORDS_PER_BLOCK)]),
               WORD_SIZE);
